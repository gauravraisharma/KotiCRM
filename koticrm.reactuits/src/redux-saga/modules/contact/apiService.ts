...
import { Contact } from '../../../models/contact/Contact';
 ...
 export function UpdateContact(id: number,contact: Contact) :Promise<apiResponse<Contact>>{ 
    return axiosInstance.post<Contact>(`/Contact/UpdateContact/${id}`,contact).then((response: AxiosResponse<Contact>) => responseBody(response)).
    catch((error:AxiosError) =>{ 
    const errorResponse: apiResponse<Contact> = {
        data:undefined,
        status: 500,
